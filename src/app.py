...
def main():
    ...
    st.markdown("""
    Загрузите текстовый файл (.txt, .pdf или .docx) для анализа русского или белорусского текста.
    ...
    lang_emoji = "🇷🇺" if lang_code == "ru" else "🇧🇾"
    lang_name = "Русский" if lang_code == "ru" else "Беларуская"
    st.info(f"{lang_emoji} **Выбранный язык:** {lang_name}")
    
    # File uploader
    uploaded_file = st.file_uploader(
        "Выберите файл",
        type=['txt', 'pdf', 'docx'],
        help="Загрузите файл .txt, .pdf или .docx с русским или белорусским текстом"
    )
    ...
    if uploaded_file is not None:
        ...
        with st.spinner("Обработка файла..."):
            try:
                ...
                # Display results
                st.markdown("---")
                analysis_lang_emoji = "🇷🇺" if lang_code == "ru" else "🇧🇾"
                analysis_lang_name = "Русский" if lang_code == "ru" else "Беларуская"
                st.header(f"📊 Результаты анализа ({analysis_lang_emoji} {analysis_lang_name})")
        ...
        with st.expander("ℹ️ О программе"):
            st.markdown("""
            ### Возможности:
            - **Многоязычная поддержка**: Русский (pymorphy3) и Белорусский (lemmatizer_be)
            ...
            2. Загрузите файл с помощью формы выше
            3. Приложение автоматически определяет тип файла и извлекает текст
            4. Текст токенизируется (разбивается на слова)
            5. Каждое слово лемматизируется с использованием соответствующей библиотеки
            6. Рассчитываются статистики и выполняется частотный анализ
            
            ### Технологии:
            - **Русский язык**: pymorphy3 с русскими словарями
            - **Белорусский язык**: lemmatizer_be (лемматизатор на основе Bnkorpus)
